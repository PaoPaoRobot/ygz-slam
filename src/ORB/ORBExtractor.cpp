...
) 
{
    ...
    for ( int i=0; i<corners.size(); i++ ) {
        ...
        kp.pt.y = corner[1];
        // the orientation 
        kp.angle = IC_Angle(image, kp.pt, _umax);
    }
...
)
{
    ...
    keypoints.angle = IC_Angle( image, keypoints.pt, _umax );
    vector<cv::KeyPoint> kp{keypoints};
    _orb->compute( image, kp, descriptors );

}
*/
float ORBExtractor::IC_Angle ( 
    const Mat& image, cv::Point2f pt, const vector< int >& u_max )
{
    int m_01 = 0, m_10 = 0;
...
void ORBExtractor::Compute ( Frame* frame )
{
    ...
    for ( auto iter = frame->_map_point_candidates.begin(); iter!=frame->_map_point_candidates.end(); iter++) {
        ...
        ComputeOrbDescriptor( kp, frame->_pyramid[0], &pattern[0], descriptor.data );
        // kp.pt.x *= (1<<level);
        // kp.pt.y *= (1<<level);
